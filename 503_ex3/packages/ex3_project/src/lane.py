...
class Follower(DTROS):

        def __init__(self, node_name):
                super(Follower, self).__init__(node_name=node_name, node_type=NodeType.DRIVER)
                self.bridge = cv_bridge.CvBridge()
                cv2.namedWindow("window", 1)

                self.image_sub = rospy.Subscriber('/csc22933/camera_node/image/compressed',
                        CompressedImage, self.image_callback)

                self.cmd_vel_pub = rospy.Publisher('/csc22933/wheels_driver_node/car_cmd',
                        Twist, queue_size=1)
...
if __name__ == '__main__':
	follower = Follower(node_name="lane_follower")
	rospy.spin()

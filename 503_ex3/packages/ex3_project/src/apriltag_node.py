    ...
    def project(self, msg):
        ...
        if len(tags) == 0:
            ...
            for tag in tags:
                ...
                print("hello the pose is",tag.pose_t)
                print("hello the tag is",tag)
            ...
            if len(distance_list) != 0:
                ...
                transform.header.frame_id = 'csc22933/camera_optical_frame'
                transform.child_frame_id = str(t)
                transform.transform.translation.x = tag.pose_t[0]
                transform.transform.translation.y = tag.pose_t[1]
                transform.transform.translation.z = tag.pose_t[2]
                R = np.vstack([np.hstack([tag.pose_R, np.reshape(tag.pose_t,(-1, 1))]), np.array([0, 0, 0, 1])])

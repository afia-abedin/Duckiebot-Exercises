...
class AprilTagNode(DTROS):
        ...
        if self.frequency_control % 10 == 0:
            ...
            if len(tags) == 0:
                ...
                if len(distance_list) != 0:
                    ...
                    apriltag_trans = self.buffer.lookup_transform('csc22933/world' ,f"{frame_name}",time=rospy.Time.now(),timeout=rospy.Duration(1.0))
                    ...
                    try:
                        trans = self.buffer.lookup_transform('csc22933/world', 'csc22933/footprint', time=rospy.Time.now(),timeout=rospy.Duration(1.0))
    ...
    def get_extrinsic_filepath(self,name):
        ...
        cali_file_folder = self.rospack.get_path('exercise3 work')+'/config/calibrations/camera_extrinsic/'
    ...
    def load_intrinsics(self):
        ...
        # cali_file_folder = '/data/yaml files/calibrations/camera_intrinsic/'
        # self.frame_id = self.veh + '/camera_optical_frame'
        # self.cali_file = cali_file_folder + self.veh + ".yaml"

        self.cali_file = self.rospack.get_path('exercise3 work') + f"/yaml files/calibrations/camera_intrinsic/{self.veh}.yaml"
